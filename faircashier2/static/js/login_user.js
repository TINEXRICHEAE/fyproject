document.addEventListener('DOMContentLoaded', function () {
  ...
  inputs.forEach(input => {
    if (input.readOnly) return;  // Skip readonly fields
  ...
  form.addEventListener('submit', function (event) {
    ...
    // Validate email (only if not readonly)
    if (!emailInput.readOnly && !validateEmail(emailInput.value)) {
      ...
      fetch(form.action, {
        ...
        .then((data) => {
          ...
          if (data.error) {
            ...
            // Use redirect_url from server response
            setTimeout(() => {
  ...
  document.addEventListener('keydown', function(e) {
    if (e.key === 'Enter' && e.target.tagName === 'INPUT') {
      ...
      if (currentIndex < inputs.length - 1 && !inputs[currentIndex + 1].readOnly) {
        e.preventDefault();
