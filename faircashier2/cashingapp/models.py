...
class PesapalTransaction(models.Model):
    """
    Stores PesaPal-specific request/response data for every order submitted
    to the PesaPal API.  See docstring in SECTION A above for full details.
    """
    id = models.AutoField(primary_key=True)

    merchant_reference  = models.CharField(max_length=255, unique=True, db_index=True)
    ...
    pesapal_status  = models.CharField(
        max_length=20, choices=PESAPAL_STATUS_CHOICES, default='PENDING')
    ...
    class Meta:
        app_label = 'cashingapp'
        db_table  = 'pesapal_transactions'
        ordering  = ['-created_at']

    def __str__(self):
        return (
            f'PesapalTxn(ref={self.merchant_reference[:12]}… '
            f'status={self.pesapal_status} amount={self.amount})'
        )
